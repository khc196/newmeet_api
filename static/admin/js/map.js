...
(function($) {
  ...
  var initial_lat = 37.558591,
      initial_lon = 126.936220;
  ...
  function displayPagination(pagination) {
    var paginationEl = document.getElementById('s_pagination'),
        fragment = document.createDocumentFragment(),
        i; 
  ...
  function initMap() {
    ...
    $prevEl.after( $('<div class="map_wrap"></div>') );
    var prev_el_selector2 = '.map_wrap';
    var $prevEl2 = $(prev_el_selector2);
    $prevEl2.append( $('<div id="map_pane" style="width:100%;height:100%;position:relative;overflow:hidden;"></div>') );
    $prevEl2.append( $('<div id="menu_wrap" class="bg_white"><div class="option"><div> <form id="search_form"> 키워드 : <input type="text" value="" id="keyword" size="15"><input type="text" style="display: none;" /> <button id="search_btn" type="button">검색하기</button> </form> </div> </div><hr><ul id="placesList"></ul><div id="s_pagination"></div></div>') );
    document.getElementById('search_btn').addEventListener("click", searchPlaces, false);
    document.getElementById('keyword').addEventListener("keypress", searchPlacesEn, false);

    var mapEl = document.getElementById('map_pane');
    var position = new daum.maps.LatLng(initial_lat, initial_lon);

    var options = {
        center: position,
        level: 3
    };
    map = new daum.maps.Map(mapEl, options);
